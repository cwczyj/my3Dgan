...
Loss = zeros((num*epoch+1) * 4,1);
Loss_D = zeros((num*epoch+1) * 4,1);
Loss_G = zeros((num*epoch+1) * 4,1);
GLoss = zeros(epoch * 4,1);
...
%         discriminator = myDiscriminator(discriminator, back_loss, 'backward', 'true');
        ...
        Loss(x*num+j + (num*epoch+1)*(total_sum-1)) = mean(tmp(:));
        
        tmp = log(d_Loss_real);
        Loss_D(x*num+j + (num*epoch+1)*(total_sum-1)) = mean(tmp(:));
        
        tmp = log(1-d_Loss_fake);
        Loss_G(x*num+j + (num*epoch+1)*(total_sum-1)) = mean(tmp(:));
        fprintf('Loss in D is %f ,Loss_D is %f and Loss_G is %f and Loss is %f\n',Loss(x*num+j + (num*epoch+1)*(total_sum-1)),...
            Loss_D(x*num+j + (num*epoch+1)*(total_sum-1)),Loss_G(x*num+j + (num*epoch+1)*(total_sum-1)),mean(d_Loss + d_gen_Loss,1));
    end
    ...
    if 1
        ...
        GLoss(x+1 + epoch*(total_sum - 1))=mean(tmp(:));
